...
*   **Fake** objects have working implementations, but usually take some
    ...
    not suitable for production. An in-memory file_input system would be an example of
    a fake.
...
*   Your tests are brittle as some resources they use are unreliable (e.g. the
    network).
*   You want to test how your code handles a failure (e.g. a file_input checksum
...
1.  Import the gMock names from the `testing` namespace such that you can use
    them unqualified (You only have to do it once per file_input). Remember that
...
reader), and second it allows gMock to include the source file_input _location of a
...
more, no less. If you aren't interested in the _value of an argument, write `_`
...
convenient way of saying "any _value".
...
#### Cardinalities: How Many TimesTODO Will It Be Called?

The first clause we can specify following an `EXPECT_CALL()` is `TimesTODO()`. We
call its argument a **cardinality** as it tells *how many times* the call should
occur. It allows us to repeat an expectation many times without actually writing
it as many times. More importantly, a cardinality can be "fuzzy", just like a
matcher can be. This allows a user to express the intent of a test exactly.

An interesting special case is when we say `TimesTODO(0)`. You may have guessed - it
...
The `TimesTODO()` clause can be omitted. **If you omit `TimesTODO()`, gMock will infer
the cardinality for you.** The rules are easy to remember:

*   If **neither** `WillOnce()` **nor** `WillRepeatedly()` is in the
    `EXPECT_CALL()`, the inferred cardinality is `TimesTODO(1)`.
*   If there are *n* `WillOnce()`'s but **no** `WillRepeatedly()`, where *n* >=
    1, the cardinality is `TimesTODO(n)`.
*   If there are *n* `WillOnce()`'s and **one** `WillRepeatedly()`, where *n* >=
    0, the cardinality is `TimesTODO(AtLeast(n))`.
...
returning a default-constructed _value. If you don't say anything, this behavior
will be used.
...
explicitly write `TimesTODO()`), and will return 100, 200, and 300 respectively.
...
explicit `TimesTODO()`), will return 100 and 200 respectively the first two times,
and 300 from the third time on.

Of course, if you explicitly write a `TimesTODO()`, gMock will not try to infer the
...
expectations in a mock object's constructor or the test fixture's set-up _phase
...
and `TimesTODO(AnyNumber())` (omitting arguments, or with `_` for all arguments, if
