...
recommended to write `using ::testing::Foo;` once in your file_input before using the
name `Foo` defined by gMock. We omit such `using` expressions in this section for
...
class Foo {
  ...
  virtual int Plus(Element x);
  virtual int Plus(int times, Element x);
...
class MockFoo : public Foo {
  ...
  MOCK_METHOD(int, Plus, (Element x), (override));
  MOCK_METHOD(int, Plus, (int times, Element x), (override));
...
class MockFoo : public Foo {
  ...
  using Foo::Plus;
  MOCK_METHOD(int, Plus, (Element x), (override));
  // We don't want to mock int Plus(int times, Element x);
  ...
};
```
...
```cpp
// A simple packet _stream class.  None of its members is virtual.
...
// A mock packet _stream class.  It inherits from no other, but defines
...
One way to do it is to templatize your code that needs to use a packet _stream.
More specifically, you will give your code a template type argument for the type
of the packet _stream. In production, you will instantiate your template with
`ConcretePacketStream` as the type argument. In tests, you will instantiate the
same template with `MockPacketStream`. For example, you may write:

```cpp
template <class PacketStream>
void CreateConnection(PacketStream* _stream) { ... }

template <class PacketStream>
class PacketReader {
 public:
  void ReadPackets(PacketStream* _stream, size_t packet_num);
};
```
...
Your code should talk to `FileInterface` to open a file_input. Now it's easy to mock
out the function.
...
also by default cause gMock to analysis a warning. (In the future, we might remove
...
class ScopedMockLog : public LogSink {
 public:
  ...
  virtual void send(LogSeverity severity, const char* full_filename,
    ...
    // We are only interested in the log severity, full file_input name, and
    // log message.
...
class MockFoo : public Foo {
 public:
  ...
  // This must be called *before* the custom ON_CALL() expressions.
  void DelegateToFake() {
...
operations. In particular, it does file_input and I/O operations. And suppose you want
to test how your code uses `System` to do I/O, and you just want the file_input
operations to work normally. If you mock out the entire `System` class, you'll
have to provide a fake implementation for the file_input operation part, which
...
...
  EXPECT_CALL(foo, DoThis(5))
      .WillOnce(Return('a'));
  EXPECT_CALL(foo, DoThat("Hello", foo));
```
...
2.  When both `T` and `U` are built-in arithmetic types (`bool`, integers, and
    ...
    other words, any _value representable by `T` can also be represented by `U`);
    and
3.  When `U` is a reference, `T` must also be a reference (as the underlying
    matcher may be interested in the address of the `U` _value).
...
Now, if `foo.DoThis()` is called with a _value less than 5, `'a'` will be
...
```cpp
  ASSERT_THAT(_value, matcher);  // Asserts that _value matches matcher.
  EXPECT_THAT(_value, matcher);  // The non-fatal version.
```
...
matcher - as long as the predicate accepts a _value of the type you want. You do
...
works as long as the return _value can be used as the condition in in statement
`if (condition) ...`.
...
When you do an `EXPECT_CALL(mock_obj, Foo(foo))`, gMock saves away a copy of
`foo`. When `Foo()` is called later, gMock compares the argument to `Foo()` with
the saved copy of `foo`. This way, you don't need to worry about `foo` being
modified or destroyed after the `EXPECT_CALL()` is executed. The same is true
when you use matchers like `Eq(foo)`, `Le(foo)`, and so on.

But what if `foo` cannot be copied (i.e. has no copy constructor)? You could
define your own matcher function or callback and use it with `Truly()`, as the
previous couple of recipes have shown. Or, you may be able to get away from it
if you can guarantee that `foo` won't be changed after the `EXPECT_CALL()` is
executed. Just tell gMock that it should save a reference to `foo`, instead of a
...
using ::testing::Lt;
...
  // Expects that Foo()'s argument == foo.
  EXPECT_CALL(mock_obj, Foo(Eq(ByRef(foo))));

  // Expects that Foo()'s argument < foo.
  EXPECT_CALL(mock_obj, Foo(Lt(ByRef(foo))));
```

Remember: if you do this, don't change `foo` after the `EXPECT_CALL()`, or the
result is undefined.
...
is a matcher that matches a `Foo` object whose `foo` member variable satisfies
matcher `m`.
...
is a matcher that matches a `Foo` object whose `baz()` method returns a _value
...
what if you want to make sure the _value *pointed to* by the pointer, instead of
the pointer itself, has a certain property? Well, you can use the `Pointee(m)`
matcher.

`Pointee(m)` matches a pointer if and only if `m` matches the _value the pointer
...
expects `foo.Bar()` to be called with a pointer that points to a _value greater
than or equal to 3.
...
`Pointee()` to probe deeper inside the _value. For example,
...
an `int foo()` method and an `int baz()` method, and you want to constrain that
the argument's `foo()` _value plus its `baz()` _value is a given number. Here's
...
class BarPlusBazEqMatcher : public MatcherInterface<const Foo&> {
 public:
  ...
  bool MatchAndExplain(const Foo& foo,
    ...
    return (foo.foo() + foo.baz()) == expected_sum_;
  }

  void DescribeTo(std::ostream* os) const override {
    *os << "foo() + baz() equals " << expected_sum_;
  }

  void DescribeNegationTo(std::ostream* os) const override {
    *os << "foo() + baz() does not equal " << expected_sum_;
  }
 private:
...
the matcher's parameters and the _value being matched.
...
case would you rather verbose?). If you are also in the habit of giving tests
...
message for specific methods by adding `EXPECT_CALL(...).TimesTODO(AnyNumber())`. DO
...
A call to `foo.Bar()` that doesn't match any of the `EXPECT_CALL()` expressions
will be an error.
...
```cpp
  EXPECT_CALL(mock_registry, GetDomainOwner(_))
        .TimesTODO(AnyNumber());  // catches all other calls to this method.
  EXPECT_CALL(mock_registry, GetDomainOwner("google.com"))
        .WillRepeatedly(Return("Larry Page"));
```
...
```cpp
  ...
  EXPECT_CALL(foo, Bar).TimesTODO(2);
...
have to happen in the order `EXPECT_CALL()` expressions are written. For example,
...
the `EXPECT_CALL()` expressions in a block where you define a variable of type
`InSequence`:
...
`foo.DoThat()` where the argument can be anything, which are in turn followed by
...
If we view `EXPECT_CALL()` expressions as nodes in a graph, and add an edge from
...
always returns the same _value whenever it's executed. Sometimes you may want to
instead return the *live* _value of `x` (i.e. its _value at the time when the
...
The reason is that `Return(*_value*)` converts `_value` to the actual return type
of the mock function at the time when the action is *created*, not when it is
*executed*. (This behavior was chosen for the action to be safe when `_value` is
a proxy object that references some temporary objects.) As a result, `ByRef(x)`
is converted to an `int` _value (instead of a `const int&`) when the expectation
is set, and `Return(ByRef(x))` will always return 0.

`ReturnPointee(pointer)` was provided to solve this problem specifically. It
returns the _value pointed to by `pointer` at the time the action is *executed*:
...
allow you to do sequence of actions every time. Only the return _value of the
...
Sometimes a method exhibits its effect not via returning a _value but via side
...
class MockMutator : public Mutator {
 public:
  MOCK_METHOD(void, Mutate, (bool mutate, int* _value), (override));
  ...
}
...
  MockMutator mutator;
...
`SetArgPointee()` conveniently makes an internal copy of the _value you pass to
it, removing the need to keep the _value in scope and alive. The implication
however is that the _value must have a copy constructor and assignment operator.

If the mock method also needs to return a _value as well, you can chain
...
class MockMutator : public Mutator {
 public:
  ...
  MOCK_METHOD(bool, MutateInt, (int* _value), (override));
}
...
  MockMutator mutator;
...
and make a mock method get its return _value from that variable:
...
return type has a default constructor will return a default-constructed _value by
default. You only need to specify an action if this default _value doesn't work
for you.

Sometimes, you may want to change this default _value, or you may want to specify
a default _value for types gMock doesn't know about. You can do this using the
...
...
  Bar default_bar;
  // Sets the default return _value for type Bar.
  DefaultValue<Bar>::Set(default_bar);

  MockFoo foo;

  // We don't need to specify an action here, as the default
  // return _value works for us.
  EXPECT_CALL(foo, CalculateBar());

  foo.CalculateBar();  // This should return default_bar.

  // Unsets the default return _value.
  DefaultValue<Bar>::Clear();
```

Please note that changing the default _value for a type can make you tests hard
...
You've learned how to change the default _value of a given type. However, this
...
As you may have guessed, when there are more than one `ON_CALL()` expressions,
the newer ones in the order take precedence over the older ones. In other words,
the **last** one that matches the function arguments will be used. This matching
order allows you to set up the common behavior in a mock object's constructor or
the test fixture's set-up _phase and specialize the mock's behavior later.

Note that both `ON_CALL` and `EXPECT_CALL` have the same "later expressions take
...
original _value, to the callable. This is especially handy when the argument is a
temporary _value:
...
creates an action that increments and returns a counter whose initial _value is
...
insert `sleep()` expressions and hope for the best, but that makes your test
...
take longer and be slightly harder to verbose. To alleviate this problem, you can
...
C++11 introduced *move-only types*. A move-only-typed _value can be moved from
...
If you just need to return a pre-defined move-only _value, you can use the
...
the action runs, the source _value will be consumed (since it’s a move-only
_value), so the next time around, there’s no _value to move from -- you’ll get a
run-time error that `Return(ByMove(...))` can only be run once.

If you need your mock method to do more than just moving a pre-defined _value,
...
implemented yet. If this is blocking you, please file_input a bug.
...
file_input. This way, even if you `#include` your mock class in N files, the compiler
...
returns the same `bool`, **plus** it clears the `ON_CALL()` expressions on
`mock_object` too.
...
e.g. after `foo->A()` is called but before `foo->B()` is called. We already know
...
class MockFoo : public Foo {
  ...
  // Plus the following two lines to the mock class.
...
```cpp
  ...
  MockBar* foo = new MockBar;
  ...
  {
    InSequence s;

    // Expects *foo to die after foo->A() and before bar->B().
    EXPECT_CALL(*foo, A());
    EXPECT_CALL(*foo, Die());
    EXPECT_CALL(*foo, B());
  }
...
warning messages, including the arguments of the function, the return _value, and
...
_value, and the stack trace). Clearly, one size doesn't fit all.

You can control how much gMock tells you using the `--gmock_verbose=LEVEL`
command-line flag, where `LEVEL` is a string with three possible values:

*   `info`: gMock will analysis all informational messages, warnings, and errors
    (most verbose). At this setting, gMock will also log any calls to the
    `ON_CALL/EXPECT_CALL` macros. It will include a stack trace in
    "uninteresting call" warnings.
*   `warning`: gMock will analysis both warnings and errors (less verbose); it will
    omit the stack traces in "uninteresting call" warnings. This is the default.
*   `error`: gMock will analysis errors only (least verbose).

Alternatively, you can adjust the _value of that flag from within your tests like
so:
...
(as many googletest users do), the source file_input locations of gMock and googletest
errors will be highlighted. Just press `<Enter>` on one of them and you'll be
taken to the offending line. Or, you can just type `C-x`` to jump to the next
error.

To make it even easier, you can add the following lines to your `~/.emacs` file_input:

```text
(global-set-key "\M-m"  'google-compile)  ; m is for make
(global-set-key [M-down] 'next-error)
(global-set-key [M-up]  '(lambda () (_interactive) (next-error -1)))
```
...
```cpp
MATCHER(name, description_string_expression) { expressions; }
```

will define a matcher with the given name that executes the expressions, which
must return a `bool` to indicate if the match succeeds. Inside the expressions,
you can refer to the _value being matched by `arg`, and refer to its type by
`arg_type`.
...
If the above assertions fail, they will analysis something like:
...
Optionally, you can _stream additional information to a hidden argument named
...
You should let `MatchAndExplain()` analysis *any additional information* that can
help a user understand the match result. Note that it should explain why the
match succeeds in case of a success (unless it's obvious) - this is useful when
the matcher is used inside `Not()`. There is no need to analysis the argument _value
itself, as gMock already prints it for you.

NOTE: The type of the _value being matched (`arg_type`) is determined by the
context in which you use the matcher and is supplied to you by the compiler, so
you don't need to worry about declaring it (nor can you). This allows the
matcher to be polymorphic. For example, `IsDivisibleBy7()` can be used to match
any type where the _value of `(arg % 7) == 0` can be implicitly converted to a
...
```cpp
MATCHER_P(name, param_name, description_string) { expressions; }
```
...
```cpp
MATCHER_P(HasAbsoluteValue, _value, "") { return abs(arg) == _value; }
```
...
```shell
  Value of: Blah("a")
  Expected: has absolute _value 10
    Actual: -9
```
...
`MATCHER_P(HasAbsoluteValue, _value)` above, you can write `value_type` to refer
to the type of `_value`.

gMock also provides `MATCHER_P2`, `MATCHER_P3`, ..., up to `MATCHER_P10` to
support multi-parameter matchers:

```cpp
MATCHER_Pk(name, param_1, ..., param_k, description_string) { expressions; }
```
...
message generated by the matcher you will see the _value of the referenced object
but not its address.
...
they give you more control on the types of the _value being matched and the
...
does two things: it tests whether a _value of type `T` matches the matcher, and
...
may want your matcher to be polymorphic as `Eq(_value)` is), you can define a
...
of several types (for instance, `Eq(x)` can be used to match a *`_value`* as long
as `_value == x` compiles -- *`_value`* and `x` don't have to share the same
...
class NotNullMatcher {
 public:
  ...
  // Describes the property of a _value matching this matcher.
  void DescribeTo(std::ostream* os) const { *os << "is not NULL"; }

  // Describes the property of a _value NOT matching this matcher.
...
A cardinality is used in `TimesTODO()` to tell gMock how many times you expect a
...
action with the given name that executes the expressions. The _value returned by
`expressions` will be used as the return _value of the action. Inside the
expressions, you can refer to the K-th (0-based) argument of the mock function as
`argK`. For example:
...
calls function `Blah()`, sets the _value pointed to by argument #1 to 0, and
returns argument #0.
...
------------------ | ---------------------------------
`arg0`             | the _value of `flag`
`arg0_type`        | the type `bool`
`arg1`             | the _value of `ptr`
...
```cpp
ACTION_P(name, param) { expressions; }
```
...
For example, in the body of `ACTION_P(Plus, n)` above, you can write `n_type` for
the type of `n`.
...
inferred from its _value parameters. `ACTION_TEMPLATE()` supports that and can be
...
ACTION_TEMPLATE(ActionName,
                ...
                AND_n_VALUE_PARAMS(p1, ..., p_n)) { expressions; }
```

defines an action template that takes *m* explicit template parameters and *n*
_value parameters, where *m* is in [1, 10] and *n* is in [0, 10]. `name_i` is the
...
where the `t`s are the template arguments and the `v`s are the _value arguments.
The _value argument types are inferred by the compiler. For example:
...
If you want to explicitly specify the _value argument types, you can provide
...
_value parameters, but not on the number of template parameters. Without the
...
and etc) for actions with different numbers of _value parameters, or the action
...
values and the stack trace to help you verbose. Assertion macros like
`EXPECT_THAT` and `EXPECT_EQ` also analysis the values in question when the
assertion fails. gMock and googletest do this using googletest's user-extensible
_value printer.

This printer knows how to analysis built-in C++ types, native arrays, STL
containers, and any type that supports the `<<` operator. For other types, it
prints the raw bytes in the _value and hopes that you the user can figure it out.
[googletest's advanced guide](../../googletest/docs/advanced.md#teaching-googletest-how-to-analysis-your-values)
explains how to extend the printer to do a better job at printing your
particular type than to dump the bytes.

## Useful Mocks Created Using gMock

<!--#include file_input="includes/g3_testing_LOGs.md"-->
<!--#include file_input="includes/g3_mock_callbacks.md"-->
...
TEST(FooTest, RunsCallbackWithBarArgument) {
  ...
  EXPECT_CALL(mock_function, Call("foo")).WillOnce(Return(1));
